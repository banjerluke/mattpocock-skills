...
It fits, but the weight shifts. The behaviours section lists what must hold at the agreed seams, and a refactor's contract at its seams is mostly "unchanged", so that list is short by construction. The implementation-decisions section is where the real content goes: it holds everything behind the seams, and that's the whole of a refactor. Let the durable architectural calls land as ADRs via [grill-with-docs](https://aihero.dev/skills-grill-with-docs) rather than trying to make the spec carry them.
